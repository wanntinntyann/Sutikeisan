...
void retu_kae(double **a, int sp, int ep, int count, int zyogen, int &score); //列入れ替えの主な関数
...
void retu_kae(double **a, int sp, int ep, int count, int zyogen, int& score)
{
	int val = 0;
	if (count >= zyogen) {
		...
		for (int i = sp; i < ep; i++) {
			if (!Visit[x[i]]) {
				vec[count] = x[i];
				Visit[x[i]] = true;
				retu_kae(a, sp, ep, count + 1, zyogen, score);
				Visit[x[i]] = false;
			}
		}
	}
