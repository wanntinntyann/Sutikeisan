...
int hyoka(double **a, int n);            //グループの個数を数える
...
void retu_kae(double **a, int count, int zyogen, int& score)
{
	...
	if (count >= zyogen) {
		...
		for (int i = 0; i < zyogen; i++) {
			if (!Visit[vec_rand[i]]) {
				vec[count] = vec_rand[i];
				Visit[vec_rand[i]] = true;
				retu_kae(a, count + 1, zyogen, score);
				Visit[vec_rand[i]] = false;
			}
		}
	}
}
...
int hyoka(double** a, int n)
{
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < N; j++) {
			val2 = arr[j][vec_rand[i]];
			arr[j][vec_rand[i]] = arr[j][vec[i]];
			arr[j][vec[i]] = val2;
		}
	}
